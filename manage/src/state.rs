...
const CASINO_INFO_KEY: &[u8] = b"casino_info";
...
pub fn casino_info(storage: &mut dyn Storage) -> Singleton<CasinoInfo> {
    singleton(storage, CASINO_INFO_KEY)
}

pub fn casino_info_read(storage: &dyn Storage) -> ReadonlySingleton<CasinoInfo> {
    singleton_read(storage, CASINO_INFO_KEY)
}
